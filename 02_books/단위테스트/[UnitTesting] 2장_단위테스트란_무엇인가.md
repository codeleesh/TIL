...
2장에서는 단위테스트에 접근하는 방법에 대해서 두 가지의 뚜렷한 견해에 대해서 소개하고 있습니다. 
...
단위테스트 정의 중 세번째 `코드 조각을 격리된 방식으로 검증한다` 는 속성으로 인하여 논란이 생기게 되었고 크게 2가지 파로 나뉘게 되었습니다.
...
여기까지 단위테스트의 대한 두 가지 견해의 대한 특성을 알아보았습니다. 샘플 코드를 통해 좀 더 알아보도록 하겠습니다.
...
- Customer가 올바르게 작동하더라도 Customer에 영향을 미치는 Store 내부에 버그가 있으면 단위 테스트에 실패할 수 있습니다.
- 테스트에서 두 클래스는 서로 격리돼 있지 않습니다.
...
- Store의 실제 상태와 관계없이 테스트가 요구하는 방식으로 요청에 응답합니다.
- 테스트는 더 이상 Store를 사용하지 않습니다.



테스트 시 검증해볼 수 있는 부분으로는 고객이 상점에서 호출을 올바르게 했는지 확인할 수 있습니다.

또한 고객이 상점으로 호출해야 하는 메서드뿐만 아니라 호출 횟수까지 검증할 수 있습니다.
